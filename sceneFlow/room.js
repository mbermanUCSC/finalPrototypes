...
class Start extends GameLevel {
    ...
    // ---------------------------------------
            ...
            if(this.location.r === 3 && this.location.c === 3) {
    ...
    gotoDoor(r, l, x, y) {
        if (!this.sceneChanged && !this.tutorial) {
            ...
            if (this.location.r === 2 && this.location.c === 3) { 
                this.goToEnding();
            }
            else {
    ...
    initializeItemLocations() {
        if(this.itemLocations.length == 0) { // if item locations hasn't been initialized yet
            let itemNames = ['note', 'harassing', 'sound', 'picture', 'blackmail', 'father', 'discouraged', 'outside', 'phone', 'breakIn', 'waiting', 'violating'];//extra items go here
            //r:0 c:1
            ...
            this.itemLocations.push(harassing);
            let sound = {
                name: itemNames[2],
                r: 4,
                c: 3,
    ...
    initializeFile(){
        if(this.location.r === 3 && this.location.c === 3){
            let Files = this.map.Levels[6][3].File;
            if(Files){
                for (let i = 0; i <= 1 ; i++){
                ...
                for(let i = 1; i<4; i++){
                    if (i === 1) {
                        if (this.hasItem('note')) {
                            this.choice1Text = this.add.text(this.w * 0.615, this.h * 0.4 + i * this.h * 0.15 - 30, '(Ending 1)').setFontSize(36);
                        ...
                        if (this.hasItem('harassing')) {
                            this.choice2Text = this.add.text(this.w * 0.65, this.h * 0.4 + i * this.h * 0.15 - 30, '(Ending 2)').setFontSize(36);
                        ...
                        if (this.hasItem('sound')) {
                            this.choice3Text = this.add.text(this.w * 0.615, this.h * 0.4 + i * this.h * 0.15 - 30, '(Ending 3)').setFontSize(36);
    ...
    createDesk(object, zone, e, chair, x, y) {
        let deskO = this.add.image(x - 40, y - 100, object).setScale(.9);
        let zoneO = this.add.image(x, y, zone).setAlpha(0).setScale(0.8, 1);
        let eSpr = this.add.image(x + 200, y - 100, e).setAlpha(0);
        let chairO = this.add.image(x - 45, y - 35, chair).setScale(.8);
    ...
    initializeDesk(){
        if(this.location.r === 3 && this.location.c === 3){
            let desk = this.map.Levels[6][3].Desk;
            if(desk){
...
const game = new Phaser.Game({
    ...
    scene: [Load, Title, Options, Info, Credits, Start, GameOver, NegativeVictory, PositiveVictory, NeutralVictory, Finale, PapersPlease],
    title: "Chase",
});